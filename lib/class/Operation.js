...
class Operation{
    constructor(id = null, guid = null, code, name, color, created) {
        this.id = id;
        this.guid = guid;
        this.code = code;
        this.name = name;
    ...
    static fromJson(json){
        return new Operation(
            json.id, json.guid, json.code, json.name, json.color, json.created
        )
    }
    ...
    static async getOperationByCode(code){
        try {
            const result = await OperationModel.findOne({
                where: { code : code }
            });
