...
class  Formula {
    constructor(id = null, guid = null, code, name, amount, is_option, includes, extendes, description = null, created)
    {
        this.id = id;
        this.guid = guid;
        this.code = code;
    ...
    static async fromJson(json) {
        let optionArray = json.includes ? json.includes: [];

        let validOption = [];
        for (let optionId of optionArray) {
            const optionResult = await Formula.getFormulaById(optionId);
            if (optionResult) {
                validOption.push(optionResult.toJson());
            }
        }

        let optionExtendsArray = json.extendes ? json.extendes: [];

        let validOptionExtends = [];
        for (let optionId of optionExtendsArray) {
            const optionResult = await Formula.getFormulaById(optionId);
            if (optionResult) {
                validOptionExtends.push(optionResult.toJson());
            }
        }
        console.log(validOptionExtends, json.extendes);

        return new Formula(
            json.id, json.guid, json.code, json.name, json.amount, json.is_option, validOption, validOptionExtends, json.description, json.created
        );
    }
    ...
    toJson(){
        return{
            ...
            extends: this.extendes? this.extendes : [],
