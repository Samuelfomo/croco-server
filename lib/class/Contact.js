...
class Contact {
    constructor( id = null, guid = null, firstname = null, lastname, location = null, language, gender, mobile, email, city, created = null) {
        this.id = id;
        this.guid = guid;
    ...
    static async fromJson(json) {
        ...
        return new Contact(
            ...
            json.language, json.gender, json.mobile, json.email, cityResult, json.created
        );
    }
    ...
    async _duplicate() {
        ...
        if (existingEntry) {
            console.log(`Found ${existingEntry} entries`);
        }
    }
    ...
    /**
     *
     * @returns {{code: null, firstname: null, lastname, location: null, language, gender, mobile, email, city: (*|null)}}
     */
    toJson() {
        return {
            code: this.guid,
            ...
            // createdAt: this.created? this.created.toISOString().split('T')[0] : new Date(Date.now()).toISOString().split('T')[0],
            // time: this.created? this.created.toISOString().split('T')[1].split('.')[0] : new Date(Date.now()).toISOString().split('T')[1].split('.')[0],
        };
    }
